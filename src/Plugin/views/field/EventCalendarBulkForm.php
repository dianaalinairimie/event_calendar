...
namespace Drupal\event_calendar\Plugin\views\field;
 ...
 *
 * @ViewsField("event_calendar_bulk_form")
 */
class EventCalendarBulkForm extends FieldPluginBase implements CacheableDependencyInterface {
   ...
   *   The language manager.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EntityTypeManagerInterface $entity_manager, LanguageManagerInterface $language_manager) {
  ...
  public function getCacheContexts() {
    return $this->languageManager->isMultilingual() ? $this->getEntityTranslationRenderer()
      ->getCacheContexts() : [];
  }
  ...
  public function viewsFormSubmit(&$form, FormStateInterface $form_state) {
    if ($form_state->get('step') == 'views_form_views_form') {
      ...
      $entities = array();
      $action = $this->actions['update_event'];
      $count = 0;
      ...
      $terms = $this->entityManager
        ...
        ->loadByProperties(['name' => $form_state->getValue('action')]);
      foreach ($terms as $term) {
        if ($term->get('vid')
            ->first()
